...
class Sky:
	def __init__(self,horizon,style = 'level', night=False):
		...
		self.bottom = pygame.image.load(f'../graphics/decoration/sky{theme}/sky_bottom.png').convert()
		self.middle = pygame.image.load(f'../graphics/decoration/sky{theme}/sky_middle.png').convert()
		...
		if self.style == 'overworld':
			palm_surfaces = import_folder(f'../graphics/overworld/palms{theme}')
			self.palms = []
			...
			cloud_surfaces = import_folder(f'../graphics/overworld/clouds{theme}')
			self.clouds = []
...
class City:
	def __init__(self,horizon,level_width,city_number):
		...
		min_y = 0
		max_y = horizon
		self.city_sprites = pygame.sprite.Group()
	...
	def draw(self,surface,shift):
		self.city_sprites.update(shift)
		self.city_sprites.draw(surface)